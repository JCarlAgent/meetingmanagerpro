...
content = content.replace(
  ...
  `let inputFullText = \`User Query: "\${query}"\\nCampaign Type Context: \${campaignType}\\n\`;
    if (campaignType === 'mailer') {
      ...
      4. Ensure targetAudience still reflects the correct demographic profile based on th      4. Ensure targetAudieriteFileSync(file, content);
console.log('Fixed edge function input string');
